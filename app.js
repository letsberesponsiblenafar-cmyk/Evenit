...
function showAgendaDetail(planId){
  ...
  const requirements=post.requiresCollegeVerification?(needsVerification?'Add your private college and enrollment details to continue.':'Your college and enrollment details are verified for this event.'):'No additional identity details are required for this event.';
  ...
  const verificationAction=needsVerification?`<button class="agenda-primary" data-agenda-verify="${escapeHtml(post.id)}">Add details to continue</button>`:'';
  const joinAction=post.interested&&!isMember&&!needsVerification?`<button class="agenda-primary" data-agenda-join="${escapeHtml(post.id)}">Join this event</button>`:'';
...
async function toggleJoin(index){const post=posts[index];if(!supabase||!currentUser){showToast('Create a profile before joining this event');signupModal.classList.add('open');return}if(!post.id){post.joined=!post.joined;renderPosts();return}const isMember=post.membershipStatus==='confirmed'||post.membershipStatus==='waitlisted';if(!isMember&&post.requiresCollegeVerification&&!collegeVerificationReady){openPlanVerification(post);return}const result=isMember?await supabase.rpc('leave_plan',{p_plan_id:post.id}):await supabase.rpc('join_plan',{p_plan_id:post.id});if(result.error){showToast(result.error.message);return}const row=rpcRow(result.data);if(isMember){showToast(row?.promoted_user_id?'You left the event. A person from the waitlist was promoted.':'You left this event');}else if(row?.status==='confirmed'){showToast(row.confirmation_memo?`You’re confirmed. ${row.confirmation_memo}`:'You’re confirmed for this event ✦')}else{showToast(`You’re on the waitlist${row?.queue_position?` at #${row.queue_position}`:''}. Confirmed guests receive the entry pass.`)}await loadPlans()}
...
const planVerificationModal=document.querySelector('#plan-verification-modal');const planVerificationForm=document.querySelector('#plan-verification-form');let pendingVerificationPlanId=null;async function openPlanVerification(post){if(!post||!currentUser)return;pendingVerificationPlanId=post.id;document.querySelector('#plan-verification-copy').textContent=`${post.title} requires your college and enrollment details before it can confirm your entry.`;planVerificationForm.reset();planVerificationModal.classList.add('open');const {data}=await supabase.from('profiles').select('college,enrollment_id').eq('id',currentUser.id).maybeSingle();if(data){planVerificationForm.college.value=data.college||'';planVerificationForm.enrollment_id.value=data.enrollment_id||'';}}document.querySelector('#close-plan-verification').onclick=()=>planVerificationModal.classList.remove('open');planVerificationModal.onclick=event=>{if(event.target===planVerificationModal)planVerificationModal.classList.remove('open')};planVerificationForm.onsubmit=async event=>{event.preventDefault();if(!currentUser||!pendingVerificationPlanId)return;const button=planVerificationForm.querySelector('[type=submit]');const data=new FormData(planVerificationForm);const college=String(data.get('college')||'').trim();const enrollmentId=String(data.get('enrollment_id')||'').trim();if(!college||!enrollmentId){showToast('Add both college and enrollment ID to continue.');return}button.disabled=true;button.textContent='Saving…';try{const {error}=await supabase.from('profiles').update({college,enrollment_id:enrollmentId}).eq('id',currentUser.id);if(error)throw error;collegeVerificationReady=true;const planId=pendingVerificationPlanId;planVerificationModal.classList.remove('open');await loadPlans();const planIndex=posts.findIndex(post=>post.id===planId);const plan=posts[planIndex];if(plan?.membershipStatus==='confirmed'||plan?.membershipStatus==='waitlisted')showAgendaDetail(planId);else if(planIndex>=0){await toggleJoin(planIndex);showAgendaDetail(planId)}showToast('Private verification saved. Your event entry is ready to continue.')}catch(error){showToast(`Could not save verification: ${error?.message||'Try again.'}`)}finally{button.disabled=false;button.innerHTML='Save and continue <span>→</span>'}};
