...
async function toggleJoin(index){const post=posts[index];if(!supabase||!currentUser){showToast('Create a profile before joining this event');signupModal.classList.add('open');return false}if(!post.id){post.joined=!post.joined;renderPosts();return true}const isMember=post.membershipStatus==='confirmed'||post.membershipStatus==='waitlisted';if(!isMember&&post.requiresCollegeVerification&&!post.verificationComplete){openPlanVerification(post);return false}const result=isMember?await supabase.rpc('leave_plan',{p_plan_id:post.id}):await supabase.rpc('join_plan',{p_plan_id:post.id});if(result.error){showToast(result.error.message);return false}const row=rpcRow(result.data);if(isMember){showToast(row?.promoted_user_id?'You left the event. A person from the waitlist was promoted.':'You left this event');}else if(row?.status==='confirmed'){showToast(row.confirmation_memo?`You’re confirmed. ${row.confirmation_memo}`:'You’re confirmed for this event ✦')}else{showToast(`You’re on the waitlist${row?.queue_position?` at #${row.queue_position}`:''}. Confirmed guests receive the entry pass.`)}await loadPlans();return true}
