...
function renderFollowingCards(followingItems){
  return followingItems.map(p=>{
      ...
      return '<article class="following-card" data-plan-id="'+escapeHtml(p.id)+'">'
        ...
        +'<div class="following-card-body"><div class="following-card-author" style="cursor:pointer" onclick="renderPublicProfile(\''+escapeHtml(p.user_id)+'\')">'
...
function renderAftermathCards(items){
  return items.map(post=>{
    ...
    return '<article class="aftermath-card" data-aftermath-id="'+escapeHtml(post.id)+'">'
      +'<div class="aftermath-header" style="cursor:pointer" onclick="renderPublicProfile(\''+escapeHtml(post.author_id)+'\')"><img class="aftermath-avatar" src="'+escapeHtml(post.avatar_url||'https://i.pravatar.cc/100?img=68')+'" alt="">'
...
const pageTemplates={
  ...
  messages:`<div class="page-header message-header"><div><p class="overline">Keep the plan moving</p><h2>Messages</h2><p>Conversations and private circles, in one calm place.</p></div><button class="message-compose" type="button" title="New message">✎</button></div><div class="msg-toggle" role="tablist"><button class="msg-tab active" data-msg-tab="primary">Primary</button><button class="msg-tab" data-msg-tab="groups">Groups</button></div><div id="msg-primary-pane" class="msg-pane"><div class="message-list"><button class="message"><img src="https://i.pravatar.cc/100?img=25" alt=""><div><strong>maya.rose</strong><p>Should we bring extra blankets for the picnic?</p></div><small>2m</small></button><button class="message"><img src="https://i.pravatar.cc/100?img=47" alt=""><div><strong>ari.makes</strong><p>That coffee walk sounds perfect.</p></div><small>1h</small></button></div><div class="message-note">Join a plan to start a new conversation.</div></div><div id="msg-groups-pane" class="msg-pane" hidden><div class="groups-heading"><div><h3>Groups</h3><p>Private circles for members only.</p></div><button class="publish-button" id="open-group-create">＋ Create</button></div><div id="groups-list" class="groups-list"></div></div>`,
  settings:`<div class="page-header"><p class="overline">Make it yours</p><h2>Settings</h2><p class="settings-intro">Control the parts of Evenit that matter to you.</p></div><div class="settings-list"><button data-settings-panel="account"><span class="settings-icon">◎</span><span><strong>Account details</strong><small>Name, username, profile and email</small></span><b>›</b></button><button data-settings-panel="notifications"><span class="settings-icon">♡</span><span><strong>Notification preferences</strong><small>Choose what reaches you</small></span><b>›</b></button><button data-settings-panel="privacy"><span class="settings-icon">◌</span><span><strong>Privacy and safety</strong><small>Profile visibility and location</small></span><b>›</b></button><button data-settings-panel="help"><span class="settings-icon">?</span><span><strong>Help center</strong><small>Answers and support</small></span><b>›</b></button></div>`,
};
...
document.addEventListener('click',e=>{const settings=e.target.closest('[data-settings-panel]');if(!settings)return;const panel=settings.dataset.settingsPanel;if(panel==='account'){document.querySelector('#edit-profile')?.click();return}renderSettingsPanel(panel);});
...
document.querySelector('#saved-menu-toggle').onclick=()=>{const menu=document.querySelector('#saved-menu');const open=menu.hidden;menu.hidden=!open;document.querySelector('#saved-menu-toggle').setAttribute('aria-expanded',String(open));};
document.addEventListener('click',e=>{const saved=e.target.closest('[data-saved-collection]');if(!saved)return;activeSavedCollection=saved.dataset.savedCollection;document.querySelector('#saved-menu').hidden=true;document.querySelector('#saved-menu-toggle').setAttribute('aria-expanded','false');setPage('saved');});
...
async function loadProfileDetails(){if(!supabase||!currentUser)return;const {data}=await supabase.from('profiles').select('college,enrollment_id,about,is_private,banner_url,avatar_url').eq('id',currentUser.id).maybeSingle();if(!data)return;if(data.banner_url){const cover=document.querySelector('.profile-cover');if(cover)cover.style.backgroundImage=`url("${data.banner_url}")`}if(data.avatar_url){const profileImage=document.querySelector('.profile-intro img');if(profileImage)profileImage.src=data.avatar_url;const navImage=document.querySelector('#nav-avatar img');if(navImage)navImage.src=data.avatar_url}if(editModal.classList.contains('open')){editForm.college.value=data.college||'';editForm.enrollment_id.value=data.enrollment_id||'';editForm.about.value=data.about||'';editForm.profile_visibility.value=data.is_private?'private':'public'}}
...
async function renderPublicProfile(profileId){
  ...
  if(!isSelf&&currentUser){
    ...
    if(intro){
      ...
      supabase.rpc('get_follow_state',{p_following_id:profile.id}).then(({data:state})=>{
      ...
      btn.onclick=async()=>{
        ...
        if(data?.status==='followed'){btn.textContent='Following';btn.classList.add('following');btn.classList.remove('requested');showToast('Following ✓');}
        else if(data?.status==='requested'){btn.textContent='Requested';btn.classList.add('requested');showToast('Follow request sent');}
        else{btn.textContent='Follow';btn.classList.remove('following','requested');showToast(data?.status==='request_cancelled'?'Request cancelled':'Unfollowed');}
      };
    }
  }
}
