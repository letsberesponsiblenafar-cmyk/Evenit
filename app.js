...
const pageTemplates={
  ...
  messages:`<div class="page-header"><p class="overline">Keep the plan moving</p><h2>Messages</h2></div><div class="msg-toggle"><button class="msg-tab active" data-msg-tab="primary">Primary</button><button class="msg-tab" data-msg-tab="groups">Groups</button></div><div id="msg-primary-pane"><div class="message-list"><div class="message"><img src="https://i.pravatar.cc/100?img=25"><div><strong>maya.rose</strong><p>Should we bring extra blankets for the picnic?</p></div><small>2m</small></div><div class="message"><img src="https://i.pravatar.cc/100?img=47"><div><strong>ari.makes</strong><p>That coffee walk sounds perfect.</p></div><small>1h</small></div><div class="empty-message">Your conversations will live here.<br><span>Join a plan to meet someone new.</span></div></div></div><div id="msg-groups-pane" style="display:none;margin-top:20px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div><h3 style="margin:0;font:700 18px -apple-system,sans-serif;letter-spacing:-0.02em">Groups</h3><p style="color:#6E6E73;font-size:12px;margin:2px 0 0">Private circles for members only.</p></div><button class="publish-button" id="open-group-create" style="width:auto;border-radius:999px;padding:8px 16px;font-size:13px">＋ Create</button></div><div id="groups-list" class="groups-list" style="margin-top:0"></div></div>`,
...
document.addEventListener('click',e=>{const tab=e.target.closest('.msg-tab');if(tab){document.querySelectorAll('.msg-tab').forEach(t=>t.classList.remove('active'));tab.classList.add('active');document.querySelectorAll('.msg-pane').forEach(p=>p.style.display='none');document.querySelector('#'+tab.dataset.msgTab+'Pane').style.display='block';}},undefined);
