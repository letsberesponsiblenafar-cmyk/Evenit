...
const EVENIT_PUBLIC_ORIGIN='https://evenit.vercel.app';
let posts = [];
...
function refreshProfileAfterAuth(){
  const profileIsOpen=Boolean(pageView&&!pageView.hidden&&document.querySelector('[data-page="profile"]')?.classList.contains('active'));
...
async function loadProfileDetails(){if(!supabase||!currentUser)return;const {data,error}=await supabase.from('profiles').select('college,enrollment_id,about,is_private,banner_url,avatar_url,neighborhood').eq('id',currentUser.id).maybeSingle();if(error){console.warn('Could not load private profile details',error);return}if(!data)return;if(data.banner_url){const cover=document.querySelector('.profile-cover');if(cover)cover.style.backgroundImage=`url("${data.banner_url}")`}if(data.avatar_url){const profileImage=document.querySelector('.profile-intro img');if(profileImage)profileImage.src=data.avatar_url;const navImage=document.querySelector('#nav-avatar img');if(navImage)navImage.src=data.avatar_url}const about=document.querySelector('.profile-about-own');if(about){about.hidden=!data.about;about.textContent=data.about||''}if(editModal.classList.contains('open')||activeWorkspace==='profile'){editForm.college.value=data.college||'';editForm.enrollment_id.value=data.enrollment_id||'';editForm.about.value=data.about||'';editForm.neighborhood.value=data.neighborhood||'';editForm.profile_visibility.value=data.is_private?'private':'public'}}
...
document.title='evenit | Make plans happen';
 ...
 async function loadSiteSettings(){if(!supabase)return;const {data}=await supabase.from('site_settings').select('*').single();if(!data)return;document.title=`${String(data.site_name||'evenit').toLowerCase()} | Make plans happen`;document.documentElement.style.setProperty('--violet',data.primary_color);document.documentElement.style.setProperty('--gold',data.accent_color);applyExperienceControls(data);document.querySelectorAll('.like').forEach(button=>button.textContent=data.reaction_icon);const notification=document.querySelector('[data-page="notifications"]');if(notification){const label=[...notification.childNodes].find(node=>node.nodeType===Node.TEXT_NODE&&node.nodeValue.includes('Notifications'));if(label)label.nodeValue=label.nodeValue.replace('Notifications',data.notification_label)}}
loadSiteSettings();
 ...
 applyAdminStyles();
 function refreshPageCopy(){if(pageView.hidden)return;const page=document.querySelector('[data-page].active')?.dataset.page;const copy={discover:[adminContent.discover_eyebrow||'Find your people',adminContent.discover_title||'Discover plans worth joining.'],notifications:[adminContent.notification_eyebrow||'Stay in the loop',adminContent.notification_title||'Notifications'],messages:[adminContent.messages_eyebrow||'Keep the plan moving',adminContent.messages_title||'Messages'],settings:[adminContent.settings_eyebrow||'Make it yours',adminContent.settings_title||'Settings']};const values=copy[page];if(values){const heading=pageView.querySelector('.page-header h2');const eyebrow=pageView.querySelector('.page-header .overline');if(heading)heading.textContent=values[1];if(eyebrow)eyebrow.textContent=values[0]}if(page==='profile'){const eyebrow=pageView.querySelector('.profile-intro .overline');const emptyTitle=pageView.querySelector('.profile-empty h3');const emptyDescription=pageView.querySelector('.profile-empty>p');const create=pageView.querySelector('#profile-post');const edit=pageView.querySelector('.edit-profile');if(eyebrow&&adminContent.profile_eyebrow)eyebrow.textContent=adminContent.profile_eyebrow;if(currentUser&&emptyTitle&&adminContent.profile_empty_title)emptyTitle.textContent=adminContent.profile_empty_title;if(currentUser&&emptyDescription&&adminContent.profile_empty_description)emptyDescription.textContent=adminContent.profile_empty_description;if(create&&adminContent.create_event_button)create.firstChild.nodeValue=`${adminContent.create_event_button} `;if(edit&&adminContent.edit_profile_button)edit.textContent=adminContent.edit_profile_button}}
...
loadProfileDetails=async function(){
  ...
  const onboardingActive=sessionStorage.getItem('evenit-profile-onboarding')==='1'||currentUser?.user_metadata?.profile_onboarding===true;
  if(completion)completion.hidden=!onboardingActive;
  if(onboardingActive&&hasAbout)sessionStorage.removeItem('evenit-profile-onboarding');
};
...
function toggleProfileMenu(trigger){
  ...
  panel.innerHTML=`
    ...
    <div class="profile-menu-label">Get the app</div>
    ...
    <a role="menuitem" href="https://github.com/letsberesponsiblenafar-cmyk/Evenit/actions/workflows/ios-build.yml" target="_blank" rel="noreferrer"><span class="profile-menu-icon">${icon('download')}</span><span class="profile-menu-copy"><strong>iOS build</strong><small>Verified build · signing required for iPhone</small></span><b aria-hidden="true">↗</b></a>
